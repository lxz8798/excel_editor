    ...
    <CollapseContainer class="form_wrap" v-if="currTempDetail && Object.keys(currTempDetail).length">
      <template #title>
        <div class="form_title" v-if="currTempDetail.name">
          ...
          <span>{{ currTempDetail.name.split('源数据-')[0] }}【</span><span><a-input size="small" placeholder="请输入项目名称" v-model:value="projectNameDefalutValue" @change="changeProjectName" /></span><span>】-源数据-{{currTempDetail.name.split('源数据-')[1]}}</span>
...
import { changeInputValueApi, clearTemplate, getFormProjectName } from '/@/api/demo/form';
  ...
  setup() {
    ...
    watch(
      ...
      () => {
        ...
        formStore.setTemplateProjectName({ templateId: currentRoute.value.meta.templateId }).then((res) => state.projectNameDefalutValue = res.projectName);
      },
